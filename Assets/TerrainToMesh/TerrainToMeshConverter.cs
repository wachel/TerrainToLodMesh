...
namespace TerrainConverter
{
    ...
    public class TerrainToMeshConverter : MonoBehaviour
    {
        ...
        //public void SetQuadTreeSaveInfo(int lod,Node root)
        //{
        //    quadTreeSaveInfo[lod] = root.ToBytes();
        //}

        public Material GetMaterial(int layer, bool bFirst)
        {
        ...
        public Texture2D BakeBaseTexture()
        {
            TerrainData td = terrain.terrainData;
        ...
        public void CreateObjects(List<MeshInfo>[,] meshes)
        {
        ...
        //5 8 7
        //0 4 1
        void AddVertices(Node node, int i00, int i10, int i01, int i11, List<VecInt2> vertices, int alphaLayer)
        {
            node.vertexIndices[0] = i00;
            node.vertexIndices[1] = i10;
            node.vertexIndices[2] = i01;
            node.vertexIndices[3] = i11;
            if (node.childs != null && node.GetValidNum(alphaLayer) > 0) {
                ...
                AddVertices(node.childs[0], node.vertexIndices[0], node.vertexIndices[4], node.vertexIndices[5], node.vertexIndices[8], vertices, alphaLayer);
                AddVertices(node.childs[1], node.vertexIndices[4], node.vertexIndices[1], node.vertexIndices[8], node.vertexIndices[7], vertices, alphaLayer);
                AddVertices(node.childs[2], node.vertexIndices[5], node.vertexIndices[8], node.vertexIndices[2], node.vertexIndices[6], vertices, alphaLayer);
                AddVertices(node.childs[3], node.vertexIndices[8], node.vertexIndices[7], node.vertexIndices[6], node.vertexIndices[3], vertices, alphaLayer);
            }
        }

        public bool IsSizeLeaf(Node root, int x, int y, int size)
        {
        ...
        public List<MeshInfo> CreateMeshes(Node root,int gridSize,int alphaLayer)
        {
            ...
            int aw = terrain.terrainData.alphamapWidth;
            int ah = terrain.terrainData.alphamapHeight;
            float[,,] alphamaps = terrain.terrainData.GetAlphamaps(0, 0, aw, ah);
            ...
            root.TraversalSize(gridSize, (Node _subRoot) => {
                ...
                AddVertices(subRoot, 0, 1, 2, 3, intVertices,alphaLayer);
                ...
                for (int i = 0; i < intVertices.Count; i++) {
                    ...
                    normals.Add(terrain.terrainData.GetInterpolatedNormal(x0, y0));
                ...
                if (indices.Count > 0) {
                    ...
                    meshes.Add(new MeshInfo(mesh, subRoot.GetValidNum(alphaLayer), subRoot.x / subRoot.size, subRoot.y / subRoot.size, alphaLayer));
                }
            });
